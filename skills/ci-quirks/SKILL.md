...
Three merge-blocking CI pipelines plus one advisory AI-assist workflow. Each has
...
### 2. System Test on Kind (GitHub Actions)

Triggers: same as above. Builds with `docker` (not podman — GitHub Actions
runners), loads into Kind via `helm/kind-action@v1.14.0`, runs full system test.
...
### 4. Fullsend (AI-Assisted) — not merge-blocking
...
- **Merge queue**: Both GitHub Actions workflows trigger on `merge_group` events.
  PRs go through a merge queue before landing on `main`.
